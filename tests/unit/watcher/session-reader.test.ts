...
describe('readSessionUsage', () => {
  ...
  it('returns zero usage for a missing file', () => {
    const usage = readSessionUsage('/nonexistent/path.jsonl');
    expect(usage.inputTokens).toBe(0);
    expect(usage.outputTokens).toBe(0);
    expect(usage.requestCount).toBe(0);
  });

  it('returns zero usage for an empty file', () => {
    fs.writeFileSync(sessionFile, '');
    const usage = readSessionUsage(sessionFile);
    expect(usage.inputTokens).toBe(0);
    expect(usage.requestCount).toBe(0);
  });
  ...
  it('sums cache creation and cache read tokens', () => {
    fs.writeFileSync(
      sessionFile,
  ...
  it('ignores user-role lines (does not count their usage)', () => {
  ...
  it('handles lines with no usage field gracefully', () => {
  ...
  it('also reads top-level usage field (alternative format)', () => {
